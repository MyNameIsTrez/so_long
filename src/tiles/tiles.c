...
/*   Updated: 2022/07/14 17:06:14 by sbos          ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
t_status	sl_instantiate_tile_frames(t_tile *tile, t_data *data)
{
	...
	while (sl_iterate_frame_count(tile->tile_kind->frame_count, data) != FINISHED)
	{
		...
		tile->frame_instances_indices[data->it.frame_index] = (t_i32)frame_instance_index;
			...
			t_data *data)
{
	...
	pixels_per_tile = (t_i32)data->texture.pixels_per_tile;
	tile->column_index = (t_i32)((t_i32)tile->column_index + columns);
	tile->row_index = (t_i32)((t_i32)tile->row_index + rows);
	while (sl_iterate_frame_count(tile->tile_kind->frame_count, data) != FINISHED)
	{
				...
				t_data *data)
{
	...
	tile->frame_index = 0;
	tile->frame_instances_indices = malloc(sizeof(t_i32) * (size_t)tile_kind->frame_count);
